...
fun calculateSortedDistanceDifferences(inputPath: String): Int{
    val (list1, list2) = splitInputToLists(inputPath)
    val sortedDifferences = list1.sorted().zip(list2.sorted()) { a, b -> abs(a - b) }
    return sortedDifferences.sum()
}

fun calculateSortedDistanceProduct(inputPath: String): Int {
    val (list1, list2) = splitInputToLists(inputPath)
    return list1.sumOf { number ->
        list2.count { it == number } * number
    }
}

fun splitInputToLists(filePath: String): Pair<List<Int>, List<Int>> {
    val pairs = Utils.readFileToList(filePath)
        .map { line ->
